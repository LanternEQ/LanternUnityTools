...
half3 VertexLighting(float3 positionWS, half3 normalWS, bool firstLight)
{
    half3 vertexLightColor = half3(0.0, 0.0, 0.0);
    ...
    for (uint lightIndex = 0u; lightIndex < lightsCount; ++lightIndex)
    {
        ...
        Light light = GetAdditionalLight(lightIndex, positionWS);
        half3 addedLightColor = LightingLambert(light.color, light.direction, normalWS);
        vertexLightColor += addedLightColor;
    }
    #endif
